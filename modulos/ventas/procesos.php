...
if (!$ventas->tieneAccesoModulo($idadministrador)) {
	echo json_encode(array(
		...
		"texto" => "No tienes permiso para consultar ventas."
	));
	exit;
}
...
try {
	switch ($proceso) {
		case "getUsuariosSucursal":
			...
			$filas = $ventas->getUsuariosPorSucursal($idsucursal, $idadministrador);
			$datos = array();
	...
	if ($e->getCode() === 1 && strpos($e->getMessage(), "|") !== false) {
		...
		file_put_contents($_SERVER["DOCUMENT_ROOT"] . "/txts/excepciones.txt", "ventas/procesos.php ($proceso): " . $e->getMessage() . " -- " . date("Y-m-d H:i:s") . PHP_EOL, FILE_APPEND);
		$respuesta = array(
			"result" => "error",
			"titulo" => "Error",
