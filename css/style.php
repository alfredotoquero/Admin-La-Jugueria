<?php
/**
 * Mecanismo genérico de theming por tenant/empresa: expone los colores de
 * la empresa en sesión como variables CSS custom properties (--primary,
 * --secondary, --fancy-color-1/2) para que el resto de las hojas de estilo
 * y las clases fancy-x-* (ver /agent_manuals/agents_FANCYS.md) las consuman
 * vía var(...). No contiene estilos específicos de ningún negocio: es un
 * fork temático de sb-admin-2.css (Start Bootstrap, MIT) con estos valores
 * inyectados donde antes había colores fijos.
 *
 * Si la sesión todavía no tiene colores de empresa configurados (tenant
 * nuevo, o antes de implementar el flujo que los guarda en BD/sesión), se
 * usan los defaults estándar de sb-admin-2/Bootstrap como fallback.
 */
include($_SERVER["DOCUMENT_ROOT"] . "/includes/session.php");
header("Content-type: text/css");
$color1 = $_SESSION["infoUsuario"]["color1"] ?? "#5D9039";
$color2 = $_SESSION["infoUsuario"]["color2"] ?? "#4A732E";
$coloractivo = $_SESSION["infoUsuario"]["coloractivo"] ?? "#F6C23E";
$estilo = $_SESSION["infoUsuario"]["estilo"] ?? "";
$primary = $_SESSION["infoUsuario"]["primary"] ?? "#5D9039";
?>
:root {
--blue: #4e73df;
--indigo: #0a0a0a;
...
background-image: -webkit-gradient(linear, left top, left bottom, color-stop(10%, <?= $color1 ?>), to(<?= $color2 ?>));
background-image: linear-gradient(180deg, <?= $color1 ?> 10%, <? $color2 ?> 100%);
background-size: cover;
}

.bg-gradient-primary {
background-color: #000000;
background-image: -webkit-gradient(linear, left top, left bottom, color-stop(10%, #F0F1F1), to(#F0F1F1));
background-image: linear-gradient(180deg, #F0F1F1 10%, #F0F1F1 100%);
background-size: cover;
}
...
/* active */
.sidebar .active, .sidebar .nav-link:hover, .sidebar .nav-link:hover i /*, .sidebar .active:active, .sidebar .active:active i, .sidebar .active:hover i */{
...
input:disabled+.slider-sm:before,
input:disabled+.slider-xs:before {
    background-color: #e6e6e6;
    /* Optional: change the knob color in disabled state */
}
...
/* Variables de color dinamicas por empresa */
.fancy-x-root {
    --fancy-color-1: <?= $color1 ?>;
    --fancy-color-2: <?= $color2 ?>;
}
...
.fancy-x-root .fancy-x-header-badge {
    font-size: 12px;
...
/* Tarjetas de resumen */
.fancy-x-root .fancy-x-summary-card {
...
.fancy-x-root .fancy-x-summary-label {
    color: #5f6b7a;
    font-size: 11px;
    font-weight: 700;
    text-transform: uppercase;
...
.fancy-x-root .fancy-x-summary-value {
    color: #1c2a3a;
    font-size: 16px;
    font-weight: 700;
    line-height: 1.25;
...
/* Tabla */
.fancy-x-root .fancy-x-table-wrap {
...
.fancy-x-root .fancy-x-table {
    margin-bottom: 0;
}

.fancy-x-root .fancy-x-table thead th {
    background: #eef3f9;
    color: #3f4f63;
...
.fancy-x-root .fancy-x-table tbody td {
    border-top: 1px solid #edf1f7;
    color: #233245;
    vertical-align: middle;
}

.fancy-x-root .fancy-x-table tbody tr:hover {
    background: #f8fbff;
}

/* Badge ID de renglon */
.fancy-x-root .fancy-x-row-id {
    display: inline-block;
    min-width: 44px;
    text-align: center;
    padding: 4px 8px;
...
/* DataTables: quitar bold del info y botones de paginación */
.dataTables_info,
...
/* DataTables: info y paginación en el mismo renglón */
